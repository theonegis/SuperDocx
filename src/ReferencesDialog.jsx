...
export function ReferencesDialog({doc,capture,insert,close}) {
 ...
 async function run(fn){if(running.current)return;running.current=true;setBusy(true);setError('');try{await fn()}catch(e){setError(e.message)}finally{running.current=false;setBusy(false)}}
 async function load(source=mode,offset=0,search=query){setSelected(null);let result;if(source==='zotero')result=await cloud.search({query:search,start:offset});else {const all=[];let at=0;while(true){const page=await doc.citations.sources.list({offset:at,limit:100});all.push(...page.items);at+=page.items.length;if(!page.items.length||at>=page.total)break;}const matches=all.filter(x=>[x.fields.title,x.fields.year,...(x.fields.authors||[]).map(a=>a.last)].join(' ').toLowerCase().includes(search.toLowerCase()));result={items:matches.slice(offset,offset+25),total:matches.length};}appliedQuery.current=search;setItems(result.items);setTotal(result.total);setStart(offset);}
 useEffect(()=>{run(async()=>{setAccount(await cloud?.status());await load('document')})},[]);
 useEffect(()=>{const fn=e=>{if(e.key==='Escape'&&!busy&&!e.isComposing&&e.keyCode!==229){e.stopPropagation();close()}};window.addEventListener('keydown',fn);return()=>window.removeEventListener('keydown',fn)},[busy]);
 ...
 return <div className="modal-backdrop"><div className="modal references-modal" role="dialog" aria-modal="true" aria-label={t('插入文献')}>
  ...
  {mode==='zotero'&&!account ? <form className="zotero-connect" onSubmit={event=>{event.preventDefault();run(async()=>{setAccount(await cloud.connect(apiKey));setApiKey('');await load('zotero')})}}>
   ...
   <div className="reference-pagination"><span>{total} {t('条文献')}</span><button className="secondary-button compact-button" disabled={busy||!start} onClick={()=>run(()=>load(mode,start-25,appliedQuery.current))}>{t('上一页')}</button><button className="secondary-button compact-button" disabled={busy||start+25>=total} onClick={()=>run(()=>load(mode,start+25,appliedQuery.current))}>{t('下一页')}</button></div>
  </>}
